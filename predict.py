...
from TaxiFareModel.params import BUCKET_NAME, PATH_TO_LOCAL_MODEL, AWS_BUCKET_TEST_PATH
...
def download_model(model_directory="PipelineTest", bucket=BUCKET_NAME, rm=True):
    ...
    storage_location = 'models/{}/versions/{}/{}'.format(MODEL_NAME, model_directory, 'model.joblib')
...
def generate_submission_csv(nrows, kaggle_upload=False):
    ...
    if kaggle_upload:
        ...
        command = f'kaggle competitions submit -c new-york-city-taxi-fare-prediction -f {name} \
            -m "{kaggle_message_submission}"'
        os.system(command)
